...
void Restaurant::livrerClient(Client * client, vector<string> commande) {
	...
	if (client->getStatut() == Prestige) {						///vérifie que le client a droit aux livraisons
		...
		tables_[INDEX_TABLE_LIVRAISON]->placerClients(1);		///Si oui lui assigner la table des livraisons 
		for (int i = 0; i < commande.size(); i++) {
			commanderPlat(commande[i], INDEX_TABLE_LIVRAISON);	///Effectuer la commande
		}
		cout << "Statut de la table de livraison (table numero " << INDEX_TABLE_LIVRAISON << ") :" << endl
...
double Restaurant::calculerReduction(Client* client, double montant, bool livraison) {
	...
	case Prestige:
		if (livraison && static_cast<ClientPrestige*>(client)->getNbPoints() > SEUIL_LIVRAISON_GRATUITE) {
			return	-(fraisTransport_[static_cast<ClientPrestige*>(client)->getAddresseCode()] +
				montant * TAUX_REDUC_PRESTIGE);
		}
	default:
		return 0;
	}
...
void Restaurant::libererTable(int id) {
	...
	for (unsigned i = 0; i < tables_.size(); ++i) {
		if (id == tables_[i]->getId()) {
			...
			double montant = tables_[i]->getChiffreAffaire();
			bool estLivraison = (i == INDEX_TABLE_LIVRAISON);

			chiffreAffaire_ += (montant + calculerReduction(client, montant, estLivraison));
